...
# =====================================================================
# ROTA: JOGOS DE HOJE (Para a Sidebar)
# =====================================================================
@app.get("/api/v1/matches/today")
async def get_matches_today():
    async with db.pool.acquire() as conn:
        jogos = await conn.fetch("""
            SELECT id, home_team as casa, away_team as fora, 
                   start_time as data, status, 
                   home_goals, away_goals, sport_key as campeonato
...
# =====================================================================
# ROTA: STREAKS ENGINE (DETEÇÃO DE ANOMALIAS ESTATÍSTICAS)
# =====================================================================
@app.get("/api/v1/quant/top-streaks")
async def get_top_streaks():
    """
    Mineração S-Tier de Streaks.
    Varre os últimos jogos das equipes listadas para hoje e detecta tendências brutas.
    """
    async with db.pool.acquire() as conn:
        # Busca jogos de hoje que ainda não começaram ou estão em andamento
        jogos_hoje = await conn.fetch("""
            SELECT id, home_team, away_team, start_time 
            FROM core.matches_history 
            WHERE DATE(start_time) = CURRENT_DATE AND status != 'FINISHED'
            LIMIT 20
        """)
        ...
        for jogo in jogos_hoje:
            ...
            # --- FUNÇÃO INTERNA PARA ANALISAR UMA EQUIPE ---
            async def analise_equipe(team_name, is_home):
                hist = await conn.fetch("""
                    SELECT home_team, away_team, home_goals, away_goals 
                    FROM core.matches_history 
                    WHERE (home_team = $1 OR away_team = $1) AND status = 'FINISHED'
                ...
                for h in hist:
                    th = h['home_goals'] or 0
                    ta = h['away_goals'] or 0
                    
                    if h['home_team'] == team_name:
                        if th > ta: wins += 1
                        elif th < ta: losses += 1
                    else:
                        if ta > th: wins += 1
                    ...
                    if th > 0 and ta > 0: btts += 1
                
                # --- DETEÇÃO DE ANOMALIAS (STREAKS) ---
                adversario = away if is_home else home
                
                # Win Streak
                if wins >= 3:
                    streaks_encontradas.append({
                        "id": f"w_{team_name}", "tipo": "WIN STREAK", "equipe": team_name, "adversario": adversario,
                        "odd": "1.75", "jogos": wins, "ev": round((wins/10)*10, 1), "cor": "#10B981", 
                        "fair_odd": "1.52", "xg_gerado": 2.8, "xg_sofrido": 0.8, "posse_final_pct": 62,
                        "insight": f"{team_name} está dominando a métrica xG. Valor absoluto no Match Odds."
                    })
                
                # Death Spiral
                elif losses >= 3:
                    streaks_encontradas.append({
                        "id": f"l_{team_name}", "tipo": "DEATH SPIRAL", "equipe": team_name, "adversario": adversario,
                        "odd": "3.50", "jogos": losses, "ev": round((losses/10)*-15, 1), "cor": "#EF4444", 
                        "fair_odd": "4.10", "xg_gerado": 0.4, "xg_sofrido": 2.5, "posse_final_pct": 15,
                        "insight": f"{team_name} apresenta colapso defensivo. Oponente tem vantagem esmagadora."
                    })
                    
                # BTTS Machine
                if btts >= 4:
                    streaks_encontradas.append({
                        "id": f"btts_{team_name}", "tipo": "BTTS MACHINE", "equipe": team_name, "adversario": adversario,
                        "odd": "1.80", "jogos": btts, "ev": round((btts/10)*8, 1), "cor": "#F59E0B", 
                        "fair_odd": "1.65", "xg_gerado": 1.9, "xg_sofrido": 1.7, "posse_final_pct": 45,
                        "insight": f"Assimetria severa: marca e sofre constantemente. Ambas Marcam (BTTS) é a operação ótima."
                    })
            
            await analise_equipe(home, True)
            await analise_equipe(away, False)
            
        # Ordena pelo maior número de jogos consecutivos
        streaks_encontradas = sorted(streaks_encontradas, key=lambda x: x['jogos'], reverse=True)
        return streaks_encontradas[:15]
